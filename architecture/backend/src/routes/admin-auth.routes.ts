...
import { authenticateAdmin, requireSystemAdmin, AuthenticatedRequest } from '../middleware/auth.middleware';
...
// 비밀번호 변경 (인증 필요)
// ============================================
router.put('/change-password', authenticateAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.adminUser) {
    ...
    const adminUser = await (prisma as any).adminUser.findUnique({ where: { id: req.adminUser.id } });
    if (!adminUser) {
      return res.status(404).json({ success: false, error: '관리자를 찾을 수 없습니다' });
    }
...
// 어드민 목록 조회
router.get('/accounts', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
// 어드민 계정 생성
router.post('/accounts', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (role === 'region_admin' && !regionId) {
      return res.status(400).json({ success: false, error: '지역 어드민은 지역을 선택해야 합니다' });
    }
    ...
    logger.info(`Admin account created: ${adminUser.username} (${adminUser.role}) by ${req.adminUser?.username}`);
...
// 어드민 계정 수정
router.put('/accounts/:id', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
// 어드민 계정 삭제
router.delete('/accounts/:id', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
// 지역 목록 조회
router.get('/regions', authenticateAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
// 지역 생성 (시스템 어드민 전용)
router.post('/regions', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { code, name, nameEn, centerLatitude, centerLongitude, addressKeywords, addressKeyword, domain, platformHours, deliveryFeeSettings, autoScrape } = req.body;
    ...
    const region = await (prisma as any).region.create({
      data: {
        ...
        deliveryFeeSettings: deliveryFeeSettings || { baseFee: 3000, perKmFee: 500, maxDistance: 5.0, freeDeliveryThreshold: 30000 },
      },
    });
    ...
    if (autoScrape && keywords) {
      ...
      (async () => {
        try {
          const { diningCodeScraperService } = await import('../services/DiningCodeScraperService');
          let totalSynced = 0;
              ...
              if (restaurants && restaurants.length > 0) {
                for (const place of restaurants) {
                  try {
                    ...
                    const existingRest = place.id ? await prisma.restaurant.findFirst({
                    ...
                    logger.error(`Failed to sync restaurant ${place.name}`, { error: syncErr.message });
                  }
                }
              }
            } catch (scrapeErr: any) {
              logger.error(`Failed to scrape for keyword "${keyword}"`, { error: scrapeErr.message });
            }
          }
          logger.info(`Auto-scrape completed for region ${region.code}: ${totalSynced} restaurants synced (inactive)`);
        } catch (bgError: any) {
          logger.error(`Background scrape failed for region ${region.code}`, { error: bgError.message });
        }
      })();
    }
...
// 지역 수정
router.put('/regions/:regionId', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { regionId } = req.params;
    const { name, nameEn, centerLatitude, centerLongitude, addressKeyword, domain, isActive, platformHours, deliveryFeeSettings } = req.body;
...
router.delete('/regions/:regionId', authenticateAdmin, requireSystemAdmin, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { regionId } = req.params;
